...
import type { RoccoPlayerAppearance } from '../../player';
import {
...
function createNetherSecurityCameraAnimations(
  frameIds: readonly string[],
  disabledFrameId: string,
  ...
  return {
    ...
    [NETHER_SECURITY_CAMERA_DISABLED_ANIMATION_ID]: {
      ...
      loop: false,
      playbackRate: 1,
      frames: [{ frameId: disabledFrameId, durationMs: 1000 }],
    },
  };
}
...
function createNetherSecurityCameraSpriteDefinition(
  ...
  return {
    ...
    animations: createNetherSecurityCameraAnimations(frameIds, disabledFrameId),
...
type NetherIntercomText = RoccoLocalization['text']['nether']['intercom'];
...
function createRevealIntercomChoices(
  text: NetherIntercomText,
  ...
  implements RoccoLevel, RoccoAppearanceCapability, RoccoNetherSecurityCameraCapability
{
  ...
  private resolveIntercomChoices(): readonly NetherIntercomChoice[] {
    ...
    if (this.intercomStage === 'first-contact') {
      return createFirstIntercomChoices(intercomText);
    }

    if (this.intercomStage === 'after-emergency') {
      return createEmergencyIntercomChoices(intercomText);
    }

    if (this.intercomStage === 'after-reveal') {
      return createRevealIntercomChoices(intercomText, this.hasShelfBeenTaken);
    }
  ...
  private updateShelfTargetDescription(): void {
    ...
    const description = this.hasShelfBeenTaken
  ...
  private clearSecurityDefeatPresentation(): void {
    ...
    this.securityDefeatSequence = undefined;
    this.leftSideExposureElapsedMs = 0;
  ...
  async mount(
    ...
    this.onRestartRequested = options.onRestartRequested ?? undefined;
    this.arrivalSequence = undefined;
    this.securityDefeatSequence = undefined;
    ...
    let initialPosition: RoccoPoint;
    if (isReturningFromNetherTwo) {
      initialPosition = { ...NETHER_FORWARD_RETURN_POSITION };
      ...
      initialPosition = { ...NETHER_ENTRY_POSITION };
    }
    ...
    await Promise.all([
      (async () => {
        try {
          await engine.audio.preloadSound(NETHER_AMBIENT_SOUND_ID);
        } catch {
      ...
      (async () => {
        try {
          await engine.audio.preloadSound(NETHER_SECURITY_ALERT_SOUND_ID);
        } catch {
    ...
    if (shouldPlayArrivalSequence && portalSprite) {
      ...
      await Promise.all([
        (async () => {
          try {
            await engine.audio.preloadSound(NETHER_ARRIVAL_PORTAL_LOOP_SOUND_ID);
          } catch {
        ...
        (async () => {
          try {
            await engine.audio.preloadSound(NETHER_ARRIVAL_SPELL_SOUND_ID);
          } catch {
