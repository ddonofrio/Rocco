import type { RoccoGridMenuItem } from '../../../engine/video/grid-menu';
import type { RoccoInventoryItem } from './types';

export interface RoccoInventoryStorageOptions {
  id: string;
  columns: number;
  rows: number;
}
...
export class RoccoInventoryStorage {
  readonly id: string;
  ...
  constructor(options: RoccoInventoryStorageOptions) {
    this.id = options.id;
  ...
  replaceItems(items: readonly RoccoInventoryItem[]): void {
  ...
  getItem(itemId: string): RoccoInventoryItem | undefined {
  ...
  canStoreItem(item: RoccoInventoryItem): boolean {
