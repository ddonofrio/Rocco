...
export interface RoccoInventoryTransferSessionOptions {
  menuId: string;
  leftStorage: RoccoInventoryStorage;
  rightStorage: RoccoInventoryStorage;
...
interface RoccoInventoryTransferMappedSlot {
  storage: RoccoInventoryStorage;
...
export class RoccoInventoryTransferSession {
  readonly menuId: string;
  readonly leftStorageId: string;
  readonly rightStorageId: string;

  private readonly leftStorage: RoccoInventoryStorage;
  private readonly rightStorage: RoccoInventoryStorage;
  ...
  constructor(options: RoccoInventoryTransferSessionOptions) {
  ...
  private buildCommittedStorageItems(
  ...
  ): { leftItems: RoccoInventoryItem[]; rightItems: RoccoInventoryItem[] } | undefined {
    const currentItemsById = new Map<string, RoccoInventoryItem>();
    ...
    const leftItems: RoccoInventoryItem[] = [];
    const rightItems: RoccoInventoryItem[] = [];
    ...
    for (const item of items) {
      ...
      seenIds.add(item.id);
      const committedItem: RoccoInventoryItem = {
        ...currentItem,
  ...
  private resolveMappedSlot(slotIndex: number): RoccoInventoryTransferMappedSlot | undefined {
    const normalizedSlotIndex = Math.max(
      0,
  ...
  private resolveStorageCenterX(side: 'left' | 'right'): number {
    const startColumn =
      side === 'left' ? 0 : this.leftStorage.columns + this.spacerColumns;
