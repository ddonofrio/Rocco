...
- `auto-scroll.test.ts` - Unit tests under `tests/console/effects/runtimes/` for the helper output and defaults.
...
`makeGraphicPlaneAutoScrollEffect()` is a convenience helper that returns a correctly shaped effect descriptor for `effects.add(...)`.

## Boundary

Cartridges manage effect instances through `effects` and target ids. The effect manager, registry, and runtime `apply()` functions stay console-owned. Adding a new built-in effect means adding a runtime here and registering it with the effect registry during runtime setup.
