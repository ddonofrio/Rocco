...
export class RoccoCursorHost {
  ...
  private metrics: RoccoViewportMetrics | undefined;
  private mounted = false;
  private previousCursorStyle = '';
  private lastClientPoint: { x: number; y: number } | undefined;
  ...
  private applyProfileStyles(): void {
    this.cursorElement.style.opacity = String(this.profile.opacity);
  ...
  private applyAttachmentStyles(): void {
    ...
    this.attachmentElement.style.opacity = String(this.attachment.opacity ?? 1);
  }
  ...
  private resolveCursorPoint(clientX: number, clientY: number): RoccoCursorPoint | undefined {
  ...
  private setVisible(isVisible: boolean): void {
    this.cursorElement.style.display = isVisible && this.profile.enabled ? 'block' : 'none';
  }
