...
import type { RoccoPlaneScene } from './planes';
import type { RoccoSpriteDefinition } from './sprites';
import { RoccoRuntimeVideoSystem } from './video-system';
...
interface VideoSystemInternals {
  ...
  resolveSceneTargets(sceneX: number, sceneY: number): {
    visibleTarget?: { instanceId: string; definitionId: string; text: string };
    target?: { instanceId: string; definitionId: string };
  };
}
