...
import { PixiRoccoSpriteRenderer, RoccoSpriteSystemSDK, type RoccoSpriteDefinition } from './sprites';
...
export class RoccoRuntimeVideoSystem implements RoccoVideoSystem {
  ...
  private displayProfile: Partial<RoccoDisplayProfile> = {};
  ...
  readonly display: RoccoVideoDisplayModule = {
    setProfile: (profile) => {
      this.displayProfile = { ...this.displayProfile, ...profile };
  ...
  async preloadPlaneScene(scene: RoccoPlaneScene): Promise<void> {
    await this.planeRenderer.preloadScene(scene);
  }
  ...
  private syncActivePlaneScene(): void {
    ...
    const scene = this.planeSDK.serializeScene(this.activePlaneSceneId);
    this.planeRenderer.sync(scene);
  }
