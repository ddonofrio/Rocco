...
import type { RoccoSpriteDefinition, RoccoSpriteWalkMap } from '../../../../src/engine/video/sprites/types';
import { RoccoSpriteSystemSDK } from '../../../../src/engine/video/sprites/system';
...
describe('RoccoSpriteSystemSDK', () => {
  ...
  it('transitions to a configured next clip when a non-loop animation completes', () => {
    const system = new RoccoSpriteSystemSDK();
    const definition = createTestDefinition();
    definition.animations.once = {
      id: 'once',
    ...
    const updated = system.getSprite(sprite.id);
    expect(updated?.animation.animationId).toBe('idle');
    expect(updated?.animation.frameIndex).toBe(0);
    expect(updated?.animation.playing).toBe(true);
  });
  ...
  it('defaults goTo target completion to face a foreground target instance', () => {
    const system = new RoccoSpriteSystemSDK();
    system.loadSpriteDefinition(createDirectionalTestDefinition());
    const sprite = system.createSpriteFromDefinition('directional-hero', {
      id: 'hero',
    ...
    const target = system.createSpriteFromDefinition('directional-hero', {
      id: 'target',
    ...
    if (commanded?.motion.command?.kind === 'move-to') {
      expect(commanded.motion.command.target).toEqual({ x: 30, y: 0 });
      expect(commanded.motion.command.options).toMatchObject({
    ...
    expect(updated?.facing).toBe('down-right');
    expect(updated?.action?.actionId).toBe('idle');
    expect(updated?.animation.animationId).toBe('idle-down-right');
  });

  it('honors faceTargetOnComplete false for goTo target completion', () => {
    const system = new RoccoSpriteSystemSDK();
    system.loadSpriteDefinition(createDirectionalTestDefinition());
    const sprite = system.createSpriteFromDefinition('directional-hero', {
      id: 'hero',
    ...
    expect(
      system.goTo(sprite.id, 40, 0, {
        targetInstanceId: target.id,
        keepDistance: 10,
        speed: 80,
        faceTargetOnComplete: false,
      }),
    ).toBe(true);
